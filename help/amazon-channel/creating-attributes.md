...
作成または更新 [!DNL Commerce] 属性を設定します。 現在のAmazon属性とリンクを確認する [!DNL Commerce] 属性 [_[!UICONTROL Attributes]_表示](./attributes-view.md) Amazonセールスチャネルのホームページ この_[!UICONTROL Action]_ 列には、属性で使用可能なアクションが表示されます。 新しい [!DNL Commerce] 属性を指定します。または、既存の [!DNL Commerce] 属性とAmazon属性へのマッピング。
