...
The _[!UICONTROL Ineligible]_「 」タブには、現在Amazonで公開されているが、現在のリストルールに基づくリストには登録されていないすべての製品のリストが表示されます。 以前の製品が適格で、リストルールが変更されて不適格になった場合、製品に関連付けられた数量は 0 に減少し、製品は_&#x200B;不適格な&#x200B;_. ただし、現在も [!DNL Amazon Seller Account].

製品をから移動するには _[!UICONTROL Ineligible]_タブで、次の操作を実行できます。 [リストルールを変更する](./listing-rules.md) を使用して、製品の資格を取得できます。
